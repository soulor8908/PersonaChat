...
> **审计补齐 (2026-07-06)**：R7 涉及 2 张新表（persona_memories、llm_call_logs）+ chat_records 表新增 3 个字段（parent_record_id、branch_index、rating），原 PRD 缺数据模型草图段落（P0 级问题），现补齐。

#### chat_records 表扩展（新增 3 字段）

| 新增字段 | 类型 | 约束 | 说明 |
|---------|------|------|------|
| parent_record_id | INTEGER | NULLABLE | 父记录 ID（重新生成时指向原记录） |
| branch_index | INTEGER | DEFAULT 0 | 分支序号（同 parent 的第 N 次重新生成） |
| rating | TEXT | NULLABLE | 评分值：'like' \| 'dislike' |

迁移 SQL：`ALTER TABLE chat_records ADD COLUMN parent_record_id INTEGER; ALTER TABLE chat_records ADD COLUMN branch_index INTEGER DEFAULT 0; ALTER TABLE chat_records ADD COLUMN rating TEXT;`

#### persona_memories 表（新建）

| 字段 | 类型 | 约束 | 说明 |
|------|------|------|------|
| id | TEXT | PRIMARY KEY | 记忆 ID（UUID） |
| user_id | TEXT | NOT NULL | 用户 ID |
| persona_id | TEXT | NOT NULL | 关联 personas.id |
| key | TEXT | NOT NULL | 记忆键（提取的事实关键词） |
| value | TEXT | NOT NULL | 记忆值（事实内容） |
| category | TEXT | DEFAULT 'fact' | 记忆类别（fact/preference/...） |
| importance | INTEGER | DEFAULT 1 | 重要性（1-5，影响注入优先级） |
| created_at | INTEGER | NOT NULL | 创建时间戳 |
| updated_at | INTEGER | NOT NULL | 更新时间戳 |

索引：`idx_memories_user_persona ON persona_memories(user_id, persona_id)`
...
| model | TEXT | NOT NULL | 模型 ID（deepseek-v3 / glm-4-flash / ...） |
| provider | TEXT | NOT NULL | 提供商（deepseek / zhipu / ...） |
...
| error_message | TEXT | NULLABLE | 错误信息（status='error' 时填充） |
| created_at | INTEGER | NOT NULL | 创建时间戳 |

索引：`idx_llm_logs_created ON llm_call_logs(created_at)`

#### 迁移策略

- chat_records：3 次 ALTER TABLE ADD COLUMN（向后兼容，旧记录新字段为 NULL/默认值）
- persona_memories / llm_call_logs：CREATE TABLE IF NOT EXISTS（ensureTables 幂等）
- 回滚：`ALTER TABLE chat_records DROP COLUMN parent_record_id/branch_index/rating` + `DROP TABLE persona_memories/llm_call_logs`
