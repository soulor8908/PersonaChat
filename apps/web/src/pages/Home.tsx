...
export function Home() {
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="p-3">
        <input
          className="w-full rounded-lg border border-surface-700 bg-surface-800 px-3 py-2 text-sm text-white placeholder-slate-400 outline-none focus:border-primary-500"
          placeholder="搜索人格..."
          value={search}
      ...
      {hot.length > 0 && (
        <div className="px-3 pb-2">
          <div className="mb-2 text-xs text-slate-400">🔥 热门推荐</div>
          <div className="flex gap-2 overflow-x-auto pb-1">
            {hot.map(p => (
              <button key={p.id} onClick={() => navigate(`/chat/${p.id}`)}
                className="w-28 flex-shrink-0 rounded-xl border border-surface-700 bg-surface-800 p-3 text-left transition-colors hover:border-primary-500/50"
              >
                ...
                <div className="mt-1 text-xs text-slate-400">{p.messageCount} 对话 · {Math.round(p.likeRate * 100)}% 好评</div>
              </button>
            ))}
          </div>
        </div>
      ...
      <div className="flex gap-1.5 overflow-x-auto px-3 pb-2">
        {CATEGORIES.map(c => (
          <button key={c.key}
            ...
            className={`whitespace-nowrap rounded-full px-3 py-1 text-xs transition-colors ${category === c.key ? 'bg-primary-500 text-white' : 'border border-surface-700 bg-surface-800 text-slate-400'}`}
          >{c.label}</button>
        ))}
      </div>
      ...
      <div className="flex gap-1.5 px-3 pb-2">
        {SORTS.map(s => (
          <button key={s.key}
            ...
            className={`rounded-full px-2 py-0.5 text-xs transition-colors ${sort === s.key ? 'bg-primary-500/10 text-primary-500' : 'text-slate-500'}`}
          >{s.label}</button>
        ))}
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-3 pb-4">
        ...
        <div className="flex flex-col gap-2">
          {list.map(p => (
            <button key={p.id} onClick={() => navigate(`/chat/${p.id}`)}
              className="flex items-center gap-3 rounded-xl border border-surface-700 bg-surface-800 p-3 text-left transition-colors hover:border-primary-500/30"
            >
              ...
              <div className="min-w-0 flex-1">
                <div className="truncate text-sm font-medium">{p.name}</div>
                <div className="truncate text-xs text-slate-400">{p.description || '暂无简介'}</div>
